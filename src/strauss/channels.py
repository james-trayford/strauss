...
class mic:
    ...
    """
    def __init__(self, phi, mic_type="directional", label="C", channel=1,  order=None, degree=None):
        self.phi = phi
        ...
        if mic_type == "directional":
            ...
            self.antenna = self._ambisonic_antenna(acn=phi)
        else:
...
class audio_channels:
    ...
    def setup_channels(self, phis, types, labels, orders=None, degrees=None):
        ...
        for i in range(self.Nmics):
            microphone = mic(phis[i], types[i], labels[i], self.channels[i], orders, degrees)
    ...
    def plot_antenna(self):
        ...
        for i in range(self.Nmics):
            ...
            antenna = microphone.antenna(plot_phis)
            normalised_volume += antenna
            p = ax.plot(plot_phis, np.clip(antenna,0, np.inf))
            p2 = ax.plot(plot_phis, np.clip(-antenna,0, np.inf),
                         c=p[0].get_color(), ls =':')
